...
class LinkWidgetProviderTransparent : AppWidgetProvider() {

    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        val prefs = context.getSharedPreferences("WidgetPrefs", Context.MODE_PRIVATE)
        val interval = prefs.getInt("sync_interval", 15).toLong()
        LinkWidgetProvider.scheduleWork(context, interval)
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == "ACTION_SYNC_WIDGET_TRANS") {
    ...
    private fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        try {
            val views = RemoteViews(context.packageName, R.layout.widget_layout_transparent)
            val prefs = context.getSharedPreferences("WidgetPrefs", Context.MODE_PRIVATE)

            views.setTextViewText(R.id.station_name, prefs.getString("station_name", "Link Arrivals"))

            val syncIntent = Intent(context, LinkWidgetProviderTransparent::class.java).apply {
                action = "ACTION_SYNC_WIDGET_TRANS"
            }
            val pendingIntent = PendingIntent.getBroadcast(context, 0, syncIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
            views.setOnClickPendingIntent(R.id.sync_button, pendingIntent)
            
            // REMOVE CLICK INTENT FROM ROOT
